(() => {
    ...
    function windowLoad() {
        ...
        const listBulets = document.querySelector(".fp-bullets");
        const header = document.querySelector(".header");
        const logo = document.querySelector(".header__box-logo");
        const page = document.querySelector(".page");
        const htmlElement = document.documentElement;
        const expertiseWrp = document.querySelectorAll("[data-expertise-wrp]");
        const headerButton = document.querySelector(".header__button-name");
        let lineActive;
        if (document.documentElement.classList.contains("fp-init")) {
            if (listBulets) {
                lineActive = document.createElement("span");
                lineActive.classList.add("active-line");
                listBulets.append(lineActive);
            }
            const fpBullets = document.querySelectorAll(".fp-bullet");
            const observer = new MutationObserver((mutations => {
                mutations.forEach((mutation => {
                    if (mutation.type === "attributes" && mutation.attributeName === "class") {
                        if (document.documentElement.classList.contains("fp-section-0")) {
                            lineActive.classList.add("active-line_hide");
                            lineActive.classList.remove("active-line_show");
                            listBulets.classList.remove("fpBuletsToShow");
                        }
                        if (document.documentElement.classList.contains("fp-section-4")) {
                            header.classList.add("dark");
                            listBulets.classList.add("dark");
                            logo.classList.add("dark");
                        }
                        if (!document.documentElement.classList.contains("fp-section-0")) {
                            if (mutation.target.classList.contains("fp-bullet-active")) {
                                const activeItem = mutation.target;
                                lineActive.style.cssText = `\t\t\t\t\n\t\t\t\t\t\t\t\ttop: ${activeItem.offsetTop - 2}px;\t\t\t\t\n\t\t\t\t\t\t\t\theight: ${activeItem.offsetHeight}px;\n\t\t\t\t\t\t\t`;
                                lineActive.classList.add("active-line_show");
                                lineActive.classList.remove("active-line_hide");
                            }
                            listBulets.classList.add("fpBuletsToShow");
                        }
                        if (!document.documentElement.classList.contains("fp-section-4")) {
                            header.classList.remove("dark");
                            listBulets.classList.remove("dark");
                            logo.classList.remove("dark");
                        }
                    }
                }));
            }));
            fpBullets.forEach((bullet => {
        ...
        if (document.documentElement.classList.contains("fp-init")) {
            const allProjects = document.querySelector(".works__body");
            const buttonProgectYear = document.querySelectorAll("[data-year]");
            const buttonMore = document.querySelector(".category-work__button-all");
            const categoryWork = document.querySelector(".category-work");
            const buttonListYears = document.querySelector(".category-work__btn-title");
            const listYears = document.querySelector(".category-work__list-years");
            const allButtonList = document.querySelectorAll(".category-work__item-year");
            buttonListYears.addEventListener("click", (function(e) {
                listYears.classList.add("_active");
                buttonListYears.classList.add("_active");
                listBulets.classList.add("hide");
                categoryWork.classList.add("shadow");
            }));
            const observerHideList = new MutationObserver((mutations => {
                mutations.forEach((mutation => {
                    if (mutation.type === "attributes" && mutation.attributeName === "class") if (listYears.classList.contains("_active")) {
                        body.addEventListener("click", (event => {
                            if (!listYears.contains(event.target) && !buttonListYears.contains(event.target)) {
                                listYears.classList.remove("_active");
                                categoryWork.classList.remove("shadow");
                                buttonListYears.classList.remove("_active");
                                function removeClass() {
                                    listBulets.classList.remove("hide");
                                }
                                setTimeout(removeClass, 200);
                            }
                        }));
                        page.dataset.noEvent = "";
                    } else delete page.dataset.noEvent;
                }));
            }));
            observerHideList.observe(listYears, {
                attributes: true,
                attributeFilter: [ "class" ]
            });
            buttonMore.addEventListener("click", (function(e) {
                allProjects.classList.remove("hide");
                categoryWork.classList.add("hide");
                listBulets.classList.remove("hide");
                categoryWork.classList.remove("shadow");
                dalateActiveItem();
            }));
            function activeItemList() {
                allButtonList.forEach((function(e) {
            ...
            buttonProgectYear.forEach((function(e) {
                e.addEventListener("click", (function(e) {
                    let activeYear = this.dataset.year;
                    allProjects.classList.add("hide");
                    categoryWork.classList.remove("hide");
                    if (activeYear) {
                        const project = document.querySelectorAll(".category-work__box-swiper");
                        project.forEach((function(e) {
                            let activesSlides = e.dataset.project;
                            let textYear = document.querySelector(".text-year");
                            const activeElement = document.querySelector(".category-work__box-swiper.sviper-active");
                            if (activeElement) e.classList.remove("sviper-active");
                            function activeItem() {
                                e.classList.toggle("sviper-active");
                                textYear.innerHTML = activeYear;
                                listBulets.classList.remove("hide");
                                buttonListYears.classList.remove("_active");
                                categoryWork.classList.remove("shadow");
                                listYears.classList.remove("_active");
                            ...
                            if (activeYear == "2018" && activesSlides == "2018") activeItem(); else if (activeYear == "2019" && activesSlides == "2019") activeItem(); else if (activeYear == "2020" && activesSlides == "2020") activeItem(); else if (activeYear == "2021" && activesSlides == "2021") activeItem(); else if (activeYear == "2022" && activesSlides == "2022") activeItem(); else if (activeYear == "2023" && activesSlides == "2023") activeItem();
                        }));
                    }
                }));
            }));
        ...
        if (document.documentElement.classList.contains("fp-init")) {
            const observer = new MutationObserver((mutations => {
                mutations.forEach((mutation => {
                    if (mutation.type === "attributes" && mutation.attributeName === "class") if (htmlElement.classList.contains("lock")) page.dataset.noEvent = ""; else delete page.dataset.noEvent;
                }));
            }));
        ...
        if (expertiseWrp) {
            expertiseWrp.forEach((function(wrp) {
                const expertiseItem = wrp.querySelectorAll("[data-expertise-item]");
                expertiseItem.forEach((function(item) {
                    item.addEventListener("click", (function() {
                        dalateActiveItem();
                        let lineExpertise = this.querySelector(".expertise__line");
                        item.nextElementSibling.classList.add("_active");
                        lineExpertise.classList.add("_active");
                        if (document.documentElement.classList.contains("fp-section-2")) deleteInfo();
                    }));
                }));
            }));
            function deleteInfo() {
                document.body.addEventListener("click", (function(event) {
                    ...
                    if (!clickedElement.closest("[data-item-button]")) dalateActiveItem();
                }));
            }
        }
        function dalateActiveItem() {
            let activeAllButtonList = document.querySelector(".category-work__item-year._active");
            let activeInfo = document.querySelector(".expertise__box-info._active");
            let activeLine = document.querySelector(".expertise__line._active");
            if (activeInfo) activeInfo.classList.remove("_active");
            if (activeAllButtonList) activeAllButtonList.classList.remove("_active");
            if (activeLine) activeLine.classList.remove("_active");
        }
        if (headerButton) {
            const textHeader = document.querySelector(".header__box-slogan");
            if (textHeader) headerButton.addEventListener("click", (function(e) {
                textHeader.classList.toggle("open");
